...
function(data, site) {
  ...
  if (data.indexOf("hs-scripts.com") != -1) {
    ...
    fs.appendFileSync("sites/hubspot.txt", site + "\n", (err) => {
      // catch error
      if (err) throw err;
...
var scriptChecks = {
  ...
  checkSF: function(data, site) {
    if (data.indexOf("sf_config") != -1 || data.indexOf("frt(") != -1) {
      fs.appendFileSync("sites/SF.txt", site + "\n", (err) => {
        if (err) throw err;
      });
  ...
  checkMarketo: function(data, site) {
    if (data.indexOf("munchkin") != -1) {
      fs.appendFileSync("sites/Marketo.txt", site + "\n", (err) => {
        if (err) throw err;
      });
  ...
  checkActon: function(data, site) {
    if (data.indexOf("acton/bn/tracker") != -1) {
      fs.appendFileSync("sites/Act-On.txt", site + "\n", (err) => {
        if (err) throw err;
      });
  ...
  checkClickDimensions: function(data, site) {
    if (data.indexOf("clickdimensions") != -1) {
      fs.appendFileSync("sites/Click-Dimensions.txt", site + "\n", (err) => {
        if (err) throw err;
      });
  ...
  checkPardot: function(data, site) {
    if (data.indexOf("pardot") != -1) {
      fs.appendFileSync("sites/Pardot.txt", site + "\n", (err) => {
        if (err) throw err;
      });
